...
class rl_attack_simulation():
    def __init__(self, mdl=mdl, train_ds=cifar10_train, test_ds=cifar10_test, norm=np.inf, xi=1e-1, step_size=1e-2,
                 input_reduction=None, action_reduction=None, **kwargs):
        self.step_size = 1e-2
        self.xi = 1e-1
        self.norm = norm
        self.mdl = mdl
    ...
    def reset_current_action(self):
        self.current_action = torch.zeros(*self.action_shape, device=device)

    def random_action(self):
        random_move = torch.zeros(*self.action_shape, device = device).uniform(-1, 1)
        return random_move
...
env = rl_attack_simulation(step_size = 1e-1, xi = 1, input_reduction = 'pca', in_pca_components = 512, action_reduction = 'geometric')
test_env = rl_attack_simulation(step_size = 1e-1, xi = 1, input_reduction = 'pca', in_pca_components = 512, action_reduction = 'geometric')

agent = TD3(env=env, test_env=test_env, ac_kwargs=dict(hidden_sizes=[256] * 2), gamma=0.99, num_test_episodes=5, max_ep_len=40)
